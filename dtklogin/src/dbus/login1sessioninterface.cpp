...
{
    QDBusPendingReply<> reply = m_interface->asyncCallWithArgumentList(QLatin1String("PauseDeviceComplete"), {major, minor});
    return reply;
}
...
{
    QDBusPendingReply<> reply = m_interface->asyncCallWithArgumentList(QLatin1String("ReleaseDevice"), {major, minor});
    return reply;
}
...
{
    QDBusPendingReply<> reply = m_interface->asyncCallWithArgumentList(QLatin1String("SetIdleHint"), {idle});
    return reply;
}
QDBusPendingReply<> Login1SessionInterface::setLockedHint(const bool locked)
{
    QDBusPendingReply<> reply = m_interface->asyncCallWithArgumentList(QLatin1String("SetLockedHint"), {locked});
    return reply;
}
QDBusPendingReply<> Login1SessionInterface::setType(const QString &type)
{
    QDBusPendingReply reply = m_interface->asyncCallWithArgumentList(QLatin1String("SetType"), {QVariant::fromValue(type)});
    return reply;
}
QDBusPendingReply<> Login1SessionInterface::takeControl(const bool force)
{
    QDBusPendingReply reply = m_interface->asyncCallWithArgumentList(QLatin1String("TakeControl"), {force});
    return reply;
}
QDBusPendingReply<int, bool> Login1SessionInterface::takeDevice(quint32 major, quint32 minor)
{
    QDBusPendingReply<int, bool> reply = m_interface->asyncCallWithArgumentList(QLatin1String("TakeDevice"), {major, minor});
    return reply;
}
